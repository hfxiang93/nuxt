...
export default function({$axios, store, app, redirect}) {
  $axios.onRequest(config => {
    ...
    if (localUrl.indexOf('BasicConfiguration') > -1) {
      url += url.indexOf('?') > -1 ? '&' : '?'
      url += `token=${store.state.token}&tenantCode=${
        store.state.tenantCode
      }&userId=${store.state.userId}&_=${new Date().getTime()}`
    } else {
      url += url.indexOf('?') > -1 ? '&' : '?'
      url += `token=${store.state.token}&mallId=${store.state.mallId}&storeId=${
        store.state.storeId
      }&tenantCode=${store.state.tenantCode}&userId=${
        store.state.userId
      }&_=${new Date().getTime()}`
    }
